...
A **Jellyfin server plugin** (C# / .NET 8) that makes individual videos publicly accessible via secret, time-limited token links, played in an embedded web player — no Jellyfin login required for viewers. It also injects a "share publicly" entry into the web client's item context menu.
...
First-time container setup: `docker run -d --name jellyshare-test -p 8096:8096 -v $TEST/config:/config -v $TEST/cache:/cache -v $TEST/media:/media jellyfin/jellyfin:10.10.7`. Complete the startup wizard via the `/Startup/*` endpoints, authenticate via `POST /Users/AuthenticateByName` (use header `X-Emby-Authorization`), then call plugin endpoints with `X-Emby-Token`.
...
- **Authorization:** use plain `[Authorize]`. The named policy `"DefaultAuthorization"` does **not** exist in Jellyfin 10.10 and throws HTTP 500.
- **Version/ABI must match the server.** Keep `Jellyfin.Controller` version in the `.csproj` and `targetAbi` in `build.yaml` aligned with the target Jellyfin release, or the plugin won't load.
